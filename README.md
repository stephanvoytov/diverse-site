...
Реальный проект, который используется бизнесом. Сайт для привлечения партнёров по франшизе польского бренда одежды **Diverse** (основан в 1993, 350+ магазинов в 10 странах). Заказчик — официальный представитель бренда в России, компания ООО «ХАУС».
...
**CMS:**
- TinaCMS (TinaCloud) — управление контентом через визуальный редактор
- 5 коллекций: Главная, О бренде, Франшиза, Коллекции, Магазины
- Контент хранится в JSON-файлах в `content/<collection>/`
- TinaCMS admin доступен на `<site>/admin/index.html`
...
| TinaCMS (TinaCloud) | Headless CMS, управление контентом |
...
| Маршрут | Описание |
|---------|----------|
| `/` | Главный лендинг франшизы: hero, пять форматов, финмодель, кейсы, FAQ |
...
- **CMS (TinaCMS)** — управление контентом через визуальный редактор или напрямую в JSON
...
| Переменная | Описание |
|------------|----------|
| `NEXT_PUBLIC_TINA_CLIENT_ID` | Client ID из TinaCloud проекта |
| `NEXT_PUBLIC_TINA_TOKEN` | Token из TinaCloud проекта |

---

## CMS (TinaCMS)

Контент управляется через **TinaCMS** (TinaCloud). Все тексты хранятся в JSON-файлах в папке `content/`:

```
content/
├── home/          # Главная страница (11 файлов)
├── about/         # О бренде (7 файлов)
├── franchise/     # Франшиза (7 файлов)
├── collection/    # Коллекции (2 файла)
└── stores/        # Магазины (3 файла)
```

**Редактирование контента:**
1. Через TinaCMS admin: `<site>/admin/index.html`
2. Напрямую в JSON-файлах (коммит → деплой)

**Важно:** После изменения `tina/config.ts` нужно обновить `tina/tina-lock.json` через `npx tinacms dev`, закоммитить и запушить — иначе TinaCloud не переиндексирует схему.
